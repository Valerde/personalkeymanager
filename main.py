...
def mymd5(key):
    h1 = hashlib.md5()
    oj = yjvi + eryv + key
    h1.update(oj.encode(encoding='utf-8'))
    # print(h1.hexdigest())
...
with open(file, 'r', encoding='utf-8') as rf:
    ...
    for i in b:
        ...
        lst1.append(e)
value0 = StringVar()
complatbox0 = ttk.Combobox(root, textvariable=value0, state='readonly')
value0.set('请选择你要修改的网站')
complatbox0.place(x=120, y=160, width=200, height=20)
complatbox0['values'] = lst1

# 请选择平台
chose_plat = Label(root, text='请选择平台')
chose_plat.place(x=10, y=280, width=100, height=20)
value = StringVar()
complatBox = ttk.Combobox(root, textvariable=value, state='readonly')
value.set('请选择你要查询的网站')
complatBox.place(x=120, y=280, width=200, height=20)
complatBox['values'] = lst1
# 查询账号
account_label = Label(root, text='账号结果')
account_label.place(x=10, y=340, width=100, height=20)
account = tkinter.StringVar(root, '')
entry_account1 = Entry(root, textvariable=account, state='readonly')
# entry_account1['state']='disabled'
entry_account1.place(x=120, y=340, width=200, height=20)
# 查询url
url_label = Label(root, text='网站链接')
url_label.place(x=10, y=310, width=100, height=20)
url = tkinter.StringVar(root, '')
entry_url1 = Entry(root, textvariable=url, state='readonly')
# entry_url1['state']='disabled'
entry_url1.place(x=120, y=310, width=200, height=20)
# 查询密码
search_label3 = Label(root, text='密码结果')
search_label3.place(x=10, y=370, width=100, height=20)
pwd = tkinter.StringVar(root, '')
entryPwd = Entry(root, textvariable=pwd, state='readonly')
# entryPwd['state']='disabled'
entryPwd.place(x=120, y=370, width=200, height=20)
    ...
    for i in b:  # i未解密
        ...
        if value0.get() == e:
            ...
            with open(file, 'a') as qf:
                qf.write(i)
    button6()
    complatbox0.set("请选择你要修改的网站")
    complatBox.set("请选择你要查询的网站")
    button7()
    ...
    with open(file, 'r', encoding='utf-8') as rf:
        ...
        for i in b:
            ...
            if value.get() == d["网站名称"]:
                ...
                pass
    button6()
    _thread.start_new_thread(clearKey, ("Thread-1", 3,))
...
def showBack(event):
    ...
    for i in b:
        ...
        if value0.get() != e:
            ...
            add_plat.set(e)
            add_url.set(d["网站链接"])
            add_account.set(d["账号"])
    button6()


complatbox0.bind("<<ComboboxSelected>>", showBack)

# 按钮处理
buttonADDpwd = tkinter.Button(root, text='增加密码', bg='white', fg='red', command=button)
buttonADDpwd.place(x=20, y=130, width=300, height=20)

buttonCHANGEpwd = tkinter.Button(root, text='修改密码', bg='white', fg='red', command=button1)
buttonCHANGEpwd.place(x=20, y=190, width=300, height=20)

buttonDELEpwd = tkinter.Button(root, text='删除密码', bg='white', fg='red', command=button4)
buttonDELEpwd.place(x=20, y=220, width=300, height=20)

buttonCreatRsa = tkinter.Button(root, text='创建秘钥', bg='white', fg='red', command=button2)
buttonCreatRsa.place(x=20, y=250, width=300, height=20)

buttonSEARCHpwd = tkinter.Button(root, text='查询密码', bg='white', fg='red', command=button3)
buttonSEARCHpwd.place(x=20, y=400, width=300, height=20)

buttonSEARCHpwd = tkinter.Button(root, text='清除所有', bg='white', fg='red', command=button7)
buttonSEARCHpwd.place(x=270, y=460, width=50, height=20)
root.mainloop()
